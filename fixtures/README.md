...
`docs/20-phase-0-common-fields.md`는 ADR-001(`docs/adr/ADR-001-phase0-contract-v0-freeze.md`)과 ADR-002(`docs/adr/ADR-002-workspace-baseline-and-b-contract-input.md`)로 Accepted 상태이며, 아래 Fixture는 Contract v0 기준선입니다. `packages/contracts`의 Zod Schema가 Machine SoT이며, Contract Test가 이 Fixture들을 검증합니다.
...
- `mock-manifest.json`의 `summaries.analyticsProviders`는 `unknown` Provider 집계를 포함 (ADR-002)
